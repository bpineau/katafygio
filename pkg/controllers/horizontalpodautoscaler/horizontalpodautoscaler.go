...
func init() {
	controllers.Register("horizontalpodautoscaler", New)
}
...
func New(conf *config.KdnConfig, ch chan<- controllers.Event) controllers.Controller {
	...
	c.ListWatch = &cache.ListWatch{
		ListFunc: func(options meta_v1.ListOptions) (runtime.Object, error) {
			return client.AutoscalingV1().HorizontalPodAutoscalers(meta_v1.NamespaceAll).List(selector)
		},
		WatchFunc: func(options meta_v1.ListOptions) (watch.Interface, error) {
			return client.AutoscalingV1().HorizontalPodAutoscalers(meta_v1.NamespaceAll).Watch(selector)
		},
	}
...
func (c *Controller) Marshal(obj interface{}) (string, error) {
	...
	return fmt.Sprintf("apiVersion: %s\nkind: %s\n%s",
		c.Conf.ClientSet.AutoscalingV1().RESTClient().APIVersion().String(),
