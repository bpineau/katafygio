...
func init() {
	controllers.Register("persistentvolumeclaim", New)
}
...
func New(conf *config.KdnConfig, ch chan<- controllers.Event) controllers.Controller {
	...
	c.ListWatch = &cache.ListWatch{
		ListFunc: func(options meta_v1.ListOptions) (runtime.Object, error) {
			return client.CoreV1().PersistentVolumeClaims(meta_v1.NamespaceAll).List(selector)
		},
		WatchFunc: func(options meta_v1.ListOptions) (watch.Interface, error) {
			return client.CoreV1().PersistentVolumeClaims(meta_v1.NamespaceAll).Watch(selector)
		},
	}
